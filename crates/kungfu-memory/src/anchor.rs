...
pub fn extract_anchors(text: &str) -> Vec<String> {
    ...
    while let Some(start) = rest.find('`') {
        ...
        if let Some(end) = rest.find('`') {
            let ident = &rest[..end];
            if ident.len() >= 2 && ident.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '.') {
...
fn is_stop_word(w: &str) -> bool {
    matches!(
        ...
        "the" | "a" | "an" | "is" | "are" | "was" | "were" | "be" | "been" | "being"
            ...
            | "it" | "its" | "of" | "in" | "to" | "for" | "with"
            | "on" | "at" | "by" | "from" | "as" | "into" | "about"
            | "not" | "no" | "but" | "or" | "and" | "if" | "then" | "else"
            | "when" | "while" | "all" | "each" | "every" | "both"
            | "use" | "using" | "used" | "also" | "which" | "who" | "what"
            | "how" | "why" | "where" | "there" | "here" | "some" | "any"
            | "other" | "more" | "most" | "such" | "only" | "own" | "same"
            | "than" | "too" | "very" | "just" | "because" | "through"
    )
}
